...
exports.getPagePosts = async (req, res) => {
  try {
    ...
    const posts = await Post.find({ pageId: id })
...
// Update page images
exports.updatePageImages = async (req, res) => {
  try {
    const { id } = req.params;
    const { profileImage, coverImage } = req.body;
    const userId = req.userId;

    console.log('🖼️ Update page images request:', { id, profileImage, coverImage, userId });
    ...
    if (coverImage !== undefined) {
      console.log('🖼️ Updating cover image:', coverImage);
    ...
    console.log('✅ Page images updated successfully');
    res.json({
      success: true,
    ...
    console.error('Error updating page images:', err);
    res.status(500).json({ error: err.message });
  }
}; 